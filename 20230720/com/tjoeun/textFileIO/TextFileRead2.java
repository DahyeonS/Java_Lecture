...
//				String 변수에 저장된 데이터를 읽어 들인다.
				Scanner scan = new Scanner(str);
//				hasNext() 메소드는 스캐너로 지정된 문자열에서 공백을 경계로 읽어들일
//				데이터가 있으면 true, 없으면 false를 리턴한다.
				while(scan.hasNext()) {
//					Scanner로 읽어들일 데이터가 int면 true, 아니면 false
					if (scan.hasNextInt()) { 
						i = scan.nextInt();
//						Scanner로 읽어들일 데이터가 boolean이면 true, 아니면 false
					} else if (scan.hasNextBoolean()) {
						b = scan.nextBoolean();
//						Scanner로 읽어들일 데이터가 double이면 true, 아니면 false
