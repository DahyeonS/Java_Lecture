...
//	이클립스가 지원하는 생성자 자동 완성 기능을 사용할 때 현재 클래스에 물리적으로 정의하지 않은
//	필드(상속받은 필드)는 생성자를 만드는 대화상자에 표시되지 않는다.
	
//	부모 클래스에서 상속받은 private 권한의 필드를 초기화하는 방법은 부모 클래스에서 상속받은
//	필드의 접근 권한이 private일 경우 자식 클래스에서 접근할 수 없으므로 부모 클래스의 생성자를
//	호출해서 초기화시킨다.
//	public ChildTest(String name, boolean gender, int age, String nickName) {
//		super(name, gender); // ParentTest(String name, boolean gender)가 실행
//		this.name = name; // 에러, 부모클래스에서 private로 설정된 필드는 접근할 수 없다.
//		this.age = age;
//		this.nickName = nickName;
//	}
	
//	부모 클래스의 private 권한으로 설정된 모든 필드의 setter가 정의되어 있다면 setter의
//	접근 권한은 메소드이므로 public을 사용하고 public 권한은 자식 클래스에서 실행할 수 있으므로
//	부모 클래스에서 상속받는 private 권한의 필드를 setter를 이용해서 초기화시킬 수 있
//	public ChildTest(String name, boolean gender, int age, String nickName) {
//		super();
//		setName(name);
//		setGender(gender);
...
//	부모 클래스에서 상속받은 필드의 접근 권한이 protected이면 자식 클래스에서 접근할 수
//	있으므로 this를 사용하여 초기화가 가능하다.
	public ChildTest(String name, boolean gender, int age, String nickName) {
		super();
		this.name = name;
		this.gender = gender;
...
//	Override(재정의): 부모 클래스로부터 상속받은 자식 클래스
	@Override
	public String toString() {
//		부모 클래스에서 상속받은 private 권한을 가지는 필드는 자식에서 접근할 수 없으므로
//		부모 클래스에서 상속받은 private 권한을 가지는 필드에 저장된 데이터를 getter 메소드를
//		사용해서 접근해야 한다.
