...
//		마방진에 채워진 숫자가 배열의 차수(n)의 배수이면 다음 숫자는 아랫줄에 채워진다. => 행 증가
//		마방진에 채워질 숫자가 배열의 차수(n)의 배수가 아니면 다음 숫자는 오른쪽 위 방향에 채워
//		진다. => 행은 1 감소 열은 1 증가
//		행이 배열의 범위를 벗어나면 마지막 행으로 수정을 하고 열이 배열의 범위를 벗어나면
//		첫번째 열로 수정한다.
		
		int[][] a = new int[5][5];
		
//		마방진을 구현할 비열이 5행 5열일 경우 최초의 "1"이 채워질 위치를 정한다.
